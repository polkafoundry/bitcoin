...
func (s *VerifyTestSuite) TestVerifyIncorrect() {
	tests := map[string]struct {
		...
		"address - different": {
			...
			expectedError: "address mismatched",
		},
		"address - invalid": {
			signedMessage: SignedMessage{
				Address:   "INVALID",
	...
	for name, tt := range tests {
		s.Run(name, func() {
			valid, err := VerifyWithChain(tt.signedMessage, &chaincfg.MainNetParams)
...
func (s *VerifyTestSuite) TestVerifyWithChainTestnet() {
	tests := map[string]SignedMessage{
		// Generated by Tobias
		"electrum legacy": {
	...
	for name, tt := range tests {
		s.Run(name, func() {
			if valid, err := VerifyWithChain(tt, &chaincfg.TestNet3Params); assert.NoError(s.T(), err) {
...
func (s *VerifyTestSuite) TestVerify() {
	...
	for i := range tests {
		s.Run(i, func() {
			if valid, err := VerifyWithChain(tests[i], &chaincfg.MainNetParams); assert.NoError(s.T(), err) {
				assert.True(s.T(), valid)
			}
		})
	}
